...
export let drawScatterPlot = (scatterPlotLayer, data, xScale, yScale, tooltip, scatterPlotWidth, scatterPlotHeight) => {
    ...
    scatterPlotLayer.selectAll('.point') //select all the circle elements with the class 'point'
      ...
      .on("mouseover", (event, d) => {
        ...
        tooltip.style("opacity", 0.9)  // Set opacity to 0.9 to show tooltip
          .html(`${d.station}`)  // Set tooltip content
          .style("left", `${event.pageX + 10}px`)  // Position the tooltip near the cursor
          .style("top", `${event.pageY + 10}px`);

        scatterPlotLayer.append("rect")
        .attr("class", "large_rect")
        .attr("x", 0)
        .attr("y", 0)
      ...
      .on('mouseout',(event, d)=>{
        ...
        tooltip.style("opacity", 0)  // Set opacity to 0.9 to show tooltip
          .html(`""`)  // Set tooltip content
          .style("left", `${event.pageX + 10}px`)  // Position the tooltip near the cursor
          .style("top", `${event.pageY + 10}px`);
        
        scatterPlotLayer.selectAll(".large_rect").remove();
